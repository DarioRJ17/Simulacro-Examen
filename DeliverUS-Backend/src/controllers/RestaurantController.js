...
const promote = async function (req, res) {
  ...
  try {
    const restaurantePromocionado = await Restaurant.findOne({ where: { userId: req.user.id, promoted: true } })

    if (restaurantePromocionado) {
      await Restaurant.update(
        { promoted: false },
        { where: { id: restaurantePromocionado.id } },
        { transaction: t }
      )
    }
